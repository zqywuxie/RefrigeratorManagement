...
router.put('/:name', authenticate, requireRoot, async (req, res) => {
  ...
  try {
    ...
    await conn.query('INSERT INTO item_types (name) VALUES (?)', [trimmedNew]);
    await conn.query('UPDATE upper_items SET item_type = ? WHERE item_type = ?', [trimmedNew, oldName]);
    await conn.query('DELETE FROM item_types WHERE name = ?', [oldName]);
